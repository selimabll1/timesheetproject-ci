pipeline {
  ...
  environment {
    ...
    // Timeouts plus généreux (push peut être long)
    DOCKER_CLIENT_TIMEOUT  = '600'
    COMPOSE_HTTP_TIMEOUT   = '600'
    DOCKER_BUILDKIT        = '1'

    // K8s (adapte si tu utilises un autre namespace)
      ...
      steps {
        script {
          ...
          sh '''
            ...
            # Important: Dockerfile doit être un vrai fichier nommé exactement "Dockerfile" (pas .txt)
            test -f Dockerfile

            # Build
            docker build --pull -t "${TAG_BUILD}" -t "${TAG_LATEST}" .
          '''
        }
      }
    }
    ...
    stage('Deploy Kubernetes') {
      steps {
        sh """
          ...
          # Namespace workshop recommandé dans ton atelier :contentReference[oaicite:0]{index=0}
